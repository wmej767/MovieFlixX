
import { useQuery } from "@tanstack/react-query";
import { getPopular, getTopRated } from "@/lib/tmdb";
import MediaGrid from "@/components/movies/MediaGrid";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";

const MoviesPage = () => {
  const { data: popularMovies, isLoading: isPopularLoading } = useQuery({
    queryKey: ['popularMoviesPage'],
    queryFn: () => getPopular('movie'),
  });

  const { data: topRatedMovies, isLoading: isTopRatedLoading } = useQuery({
    queryKey: ['topRatedMoviesPage'],
    queryFn: () => getTopRated('movie'),
  });

  return (
    <div className="container py-8">
      <h1 className="text-3xl font-bold mb-8">Movies</h1>
      <Tabs defaultValue="popular" className="w-full">
        <TabsList className="grid w-full grid-cols-2 md:w-[400px]">
          <TabsTrigger value="popular">Popular</TabsTrigger>
          <TabsTrigger value="top-rated">Top Rated</TabsTrigger>
        </TabsList>
        <TabsContent value="popular" className="mt-8">
          <MediaGrid items={popularMovies?.results} isLoading={isPopularLoading} />
        </TabsContent>
        <TabsContent value="top-rated" className="mt-8">
          <MediaGrid items={topRatedMovies?.results} isLoading={isTopRatedLoading} />
        </TabsContent>
      </Tabs>
    </div>
  );
};

export default MoviesPage;
